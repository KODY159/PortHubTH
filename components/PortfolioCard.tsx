...
const RESULT_COLORS: Record<string, { bg: string; text: string; border: string; dot: string }> = {
...
const THUMB_LETTERS = ["#8B4513","#1E6B8A","#5C7A3E","#8B5C20","#7B5EA7","#A04060"];
...
export default function PortfolioCard({
  ...
  faculty, university, result, initialSaved = false, priority
}: any) {
  const idx = hashStr(id ?? title ?? "0") % THUMB_GRADIENTS.length;
  const rs  = result ? RESULT_COLORS[result] : null;
  const cc  = category ? (CATEGORY_COLORS[category] ?? DEFAULT_CAT) : DEFAULT_CAT;
  ...
  async function handleSave(e: React.MouseEvent) {
    ...
    const next = !saved;
    setSaved(next); setSaveLoading(true);
    if (!next) {
      const { error } = await supabase.from("saved_portfolios").delete()
        .eq("user_id", userId).eq("portfolio_id", id);
      if (error) setSaved(true);
    } else {
      const { error } = await supabase.from("saved_portfolios")
    ...
    userId ? (
      <button onClick={handleSave} disabled={saveLoading}
        ...
        style={{
          ...
          width: 30, height: 30,
          background: saved ? "rgba(245,240,232,0.95)" : "rgba(245,240,232,0.80)",
          border: `1px solid ${saved ? "#C4581F" : "rgba(26,23,20,0.15)"}`,
          display: "flex", alignItems: "center", justifyContent: "center",
          cursor: "pointer", opacity: saveLoading ? 0.5 : 1,
          transition: "all 0.2s",
        }}>
  ...
  return (
    <>
      <style>{`
        ...
        .p-card-body { padding: 12px 14px 14px; display: flex; flex-direction: column; gap: 8px; flex: 1; }
        ...
        .p-user-row { display: flex; align-items: center; gap: 6px; margin-top: 2px; }
        .p-user-avatar {
        ...
        .p-view-link { margin-top: auto; font-size: 10px; color: #C4581F; letter-spacing: 0.05em; text-transform: uppercase; transition: letter-spacing 0.2s; }
        .p-card:hover .p-view-link { letter-spacing: 0.1em; }

        .p-thumb-fade { position: absolute; bottom: 0; left: 0; right: 0; height: 40px; background: linear-gradient(to top, #F5F0E8, transparent); pointer-events: none; }
      `}</style>

      <Link href={`/portfolio/${id}`} style={{ textDecoration: "none", color: "inherit" }}>
        <div className="p-card">
          {/* Thumbnail */}
          {cover_url ? (
            <div className="p-card-thumb">
              <Image src={cover_url} alt="cover" fill priority={priority}
                sizes="(max-width: 640px) 100vw, (max-width: 1024px) 50vw, 25vw"
                className="object-cover" />
              <div className="p-thumb-fade" />
              {rs && (
                <div className="p-result-badge" style={{ position:"absolute",top:8,left:8, background:rs.bg, color:rs.text, borderColor:rs.border }}>
                  <span className="p-result-dot" style={{ background:rs.dot }} />
                  {result}
                </div>
              )}
              <SaveBtn />
            ...
            <div className="p-card-thumb" style={{ background: THUMB_GRADIENTS[idx], display:"flex", alignItems:"center", justifyContent:"center", fontSize:48, fontFamily:"'Playfair Display',serif", color: THUMB_LETTERS[idx], fontWeight:500 }}>
              {title?.charAt(0).toUpperCase()}
              <div className="p-thumb-fade" style={{ background: `linear-gradient(to top, #F5F0E8, transparent)` }} />
              {rs && (
                <div className="p-result-badge" style={{ position:"absolute",top:8,left:8, background:rs.bg, color:rs.text, borderColor:rs.border }}>
                  <span className="p-result-dot" style={{ background:rs.dot }} />
                  {result}
          ...
          <div className="p-card-body">
            ...
            {/* Category + Faculty + University badges */}
            <div className="p-badge-row">
              {category && (
                <span className="p-badge" style={{ background: cc.bg, color: cc.text, borderColor: cc.border }}>
                  {category}
                </span>
              )}
              {faculty && (
                <span className="p-badge" style={{ background: "#F5EDDF", color: "#8B4513", borderColor: "#D4AA78" }}>
                  {faculty}
                </span>
              )}
              {university && (
                <span className="p-badge" style={{ background: "#EDE8DC", color: "#4A4640", borderColor: "#C8BFA8" }}>
                  {university}
                </span>
              )}
            </div>
            ...
            <div className="p-user-row">
              {userProfile ? (
                <Image src={userProfile} alt="avatar" width={18} height={18}
                  className="rounded-full object-cover" style={{ width:18,height:18,borderRadius:"50%" }} />
              ) : (
                <div className="p-user-avatar">{username?.charAt(0).toUpperCase() ?? "A"}</div>
              )}
