...
export default function EditPortfolioPage() {
  ...
  useEffect(() => {
    const init = async () => {
      ...
      setApplyYear(data.apply_year ? String(data.apply_year) : "");
      setApplyRound(data.apply_round ?? "");
  ...
  async function handleSave() {
    ...
    if (newCoverFile) {
      ...
      if (newCoverFile.size > 8 * 1024 * 1024) {
    ...
    try {
      ...
      if (newCoverFile) {
        ...
        const newPath = `${userId}/${Date.now()}.${coverExt}`;
  ...
  if (notOwner)
    return (
      ...
      >
        <div style={{ fontSize: 14, color: "#8B1A14" }}>
          คุณไม่มีสิทธิ์แก้ไขพอร์ตโฟลิโอนี้
        </div>
        <Link
          href="/profile"
          style={{
    ...
    <>
      ...
      <div className="up-root">
        ...
        <nav className="up-nav">
          ...
          <Link href={`/portfolio/${id}`} className="up-nav-back">
            ← Back to portfolio
          </Link>
        </nav>
          ...
          <div className="up-form">
                ...
                {oldCoverUrl && (
                  <div className="up-current-file">
                    ...
                    <img
                      src={oldCoverUrl}
                      alt="cover"
                      className="up-current-thumb"
                    />
                    <div className="up-current-info">
                      <span className="up-current-label">รูปปกปัจจุบัน</span>
                      <span className="up-current-badge">
                        {newCoverFile ? "⚠ จะถูกแทนที่" : "ใช้รูปนี้อยู่"}
                      </span>
                    </div>
                  </div>
                )}
                ...
                >
                  {newCoverFile ? (
                    <>
                      ...
                      <span className="up-drop-filename">
                        {newCoverFile.name}
                      </span>
                      <span className="up-drop-hint">คลิกเพื่อเปลี่ยน</span>
                    </>
                  ) : (
                    <>
                      <svg
                      ...
                      <span className="up-drop-txt">
                        คลิกเพื่อเลือกรูปปกใหม่
                      </span>
                      <span className="up-drop-hint">
                        ไม่เกิน 8MB · ถ้าไม่เลือกจะใช้รูปเดิม
                      </span>
                    </>
                  )}
                  <input
              ...
              <div className="up-file-zone">
                ...
                {oldPdfUrl && (
                  <div className="up-current-file">
                    ...
                    <div className="up-current-info">
                      <span className="up-current-label">PDF ปัจจุบัน</span>
                      <span className="up-current-badge">
                ...
                >
                  {newPdfFile ? (
                    ...
                    <>
                      ...
                      <span className="up-drop-txt">
                        คลิกเพื่อเลือก PDF ใหม่
                      </span>
                      <span className="up-drop-hint">
                        ไม่เกิน 20MB · ถ้าไม่เลือกจะใช้ไฟล์เดิม
                      </span>
                    </>
                  )}
                  <input
            ...
            {success && (
              <div className="up-success">
                ...
                <a
                  href={`/portfolio/${id}`}
                  style={{ color: "#1A5C2E", marginLeft: 8 }}
                >
                  ดูพอร์ต →
                </a>
              </div>
            ...
            <div className="up-actions">
              <Link href={`/portfolio/${id}`} className="up-cancel">
                ยกเลิก
              </Link>
              <button
                className="up-submit"
