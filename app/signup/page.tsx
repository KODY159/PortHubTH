...
export default function SignupPage() {
  ...
  async function handleSignup() {
    ...
    if (signUpData.user) {
      await supabase.from("profiles").insert({ id: signUpData.user.id, name: email.split("@")[0] });
    }
    router.push(next);
  }
    ...
    <>
      ...
      <div className="sg-root">
        ...
        <div className="sg-card">
          ...
          <input className="sg-input" type="email" placeholder="your@email.com"
          ...
          <p className="sg-footer">
            มีบัญชีแล้ว?{" "}
            <Link href={`/login?next=${next}`} className="sg-link">Log in</Link>
          </p>
        </div>
      </div>
    </>
