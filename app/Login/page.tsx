...
export default function LoginPage() {
  ...
  async function handleLogin() {
    ...
    if (error) { setError("Email หรือ Password ไม่ถูกต้อง"); setLoading(false); return; }
    router.push(next);
  }
      ...
      <div className="pa-root">
        ...
        <div className="pa-card">
          ...
          <div className="pa-logo-row">
            ...
            <span className="pa-logo-text">ThaiUniversityPorts.io</span>
          </div>
          ...
          <input className="pa-input" type="email" placeholder="your@email.com"
            value={email} onChange={e => setEmail(e.target.value)} />

          <label className="pa-label">Password</label>
          <input className="pa-input" type="password" placeholder="••••••••"
            value={password} onChange={e => setPassword(e.target.value)}
            onKeyDown={e => e.key === "Enter" && handleLogin()} />

          <button className="pa-submit" onClick={handleLogin} disabled={loading}>
            {loading ? "กำลังเข้าสู่ระบบ..." : "Log in"}
          </button>

          <p className="pa-footer-txt">
            ยังไม่มีบัญชี?{" "}
            <Link href="/signup" className="pa-footer-link">Sign up</Link>
          </p>
        </div>
      </div>
    </>
